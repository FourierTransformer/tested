...
local function run_with_cleanup(test_file, options)
   ...
   local test_results = test_module:_run(options, test_file)
...
function test_runner.run_tests(
   ...
   if test_options.random then shared.fisher_yates_shuffle(test_files) end
   ...
   local output = shared.combine_results(tests_output)
