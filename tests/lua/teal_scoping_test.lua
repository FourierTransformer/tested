...
-- the tag is because the tested_core_test loads the tl loader (so it can run teal tests), so this test passes.
t:test("lua test files cannot accidentally load teal-only modules", {tags={"teal_loader"}}, function()
