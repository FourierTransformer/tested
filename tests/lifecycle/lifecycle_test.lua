...
t:test("before_each and after_each do not run for skipped tests", function()
...
t:test("before_each is one ahead of after_each within a running test body", function()
...
t:test("after has not run during any test", function()
    tested.assert({ given = "after count",  should = "be 0", expected = 0, actual = counts.after })
    tested.assert({ given = "before count", should = "be 1", expected = 1, actual = counts.before })
end)
